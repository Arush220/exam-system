...
contract DecentralizedExamSystem {
    ...
    function updateExamDetails(uint256 _examId, uint256 _newStartTime, uint256 _newDuration) external onlyAdmin {
        require(exams[_examId].isActive, "Cannot update inactive exam");
        exams[_examId].startTime = _newStartTime;
        exams[_examId].duration = _newDuration;
        emit ExamUpdated(_examId);
    }

    function deleteExam(uint256 _examId) external onlyAdmin {
        require(!exams[_examId].isActive, "Close the exam before deleting");
        delete exams[_examId];
        emit ExamDeleted(_examId);
    }

    function getStudentAnswer(uint256 _examId, address _student) external view onlyAdmin returns (string memory) {
        require(hasSubmitted[_examId][_student], "Student has not submitted an answer");
        return submissions[_examId][_student];
    }
}
